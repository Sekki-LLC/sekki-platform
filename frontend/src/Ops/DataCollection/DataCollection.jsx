...
const DataCollection = () => {
  // Data Collection execution structure
  const [dataCollectionData, setDataCollectionData] = useState({
    ...
    // Collection Setup
    collectionSetup: {
      dataType: 'continuous', // 'continuous', 'discrete', 'attribute', 'mixed'
      collectionMethod: 'manual', // 'manual', 'automated', 'hybrid'
      frequency: 'real-time', // 'real-time', 'batch', 'scheduled'
      targetSampleSize: 30,
      currentSampleSize: 0,
      purpose: '',
      process: '',
    ...
    validation: {
      ...
      missingData: [],
      duplicates: [],
      validationStatus: 'pending' // 'pending', 'in-progress', 'completed'
    },
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      id: 1,
      type: 'ai',
      content: "Welcome to Data Collection! I'll help you execute your data collection plan efficiently and ensure data quality. Data collection is critical for reliable analysis - let's make sure we gather accurate, complete, and timely data. What data are you collecting today?",
      timestamp: new Date()
    }
  ]);
  ...
  const generateAIResponse = (userMessage) => {
    const responses = {
      'data collection': "Effective data collection requires clear operational definitions, consistent methods, and real-time validation. Focus on accuracy, completeness, and timeliness. Document any deviations immediately.",
      'data quality': "Data quality has 5 dimensions: Completeness (no missing values), Accuracy (correct values), Consistency (uniform format), Timeliness (collected when needed), Validity (meets requirements). Monitor all dimensions.",
      'validation': "Validate data in real-time using range checks, format validation, logical consistency, and outlier detection. Set up automated validation rules to catch errors immediately during collection.",
      'outliers': "Outliers may indicate measurement errors or special causes. Investigate immediately: verify measurement, check equipment calibration, review process conditions. Don't automatically remove - understand first.",
      'sample size': "Ensure adequate sample size for your analysis. For means: n≥30 for normality. For proportions: np≥5 and n(1-p)≥5. For capability: n≥100. Consider power analysis for hypothesis testing.",
      'missing data': "Missing data can bias results. Identify patterns: Missing Completely at Random (MCAR), Missing at Random (MAR), or Missing Not at Random (MNAR). Document reasons and consider imputation strategies.",
      'measurement': "Ensure measurement system is capable: conduct Gage R&R, verify calibration, train operators, use standard procedures. Measurement error should be <10% of process variation.",
      'real time': "Real-time collection enables immediate feedback and correction. Set up validation rules, monitor data quality dashboards, and establish escalation procedures for quality issues.",
      'default': "I can help with data collection execution, validation rules, quality monitoring, outlier detection, sample size adequacy, or measurement system issues. What specific aspect needs attention?"
    };
  ...
  const removeDataEntry = (entryId) => {
    setDataCollectionData(prev => ({
      ...prev,
      dataEntries: prev.dataEntries.filter(entry => entry.id !== entryId),
      collectionSetup: {
        ...prev.collectionSetup,
        currentSampleSize: prev.dataEntries.filter(entry => entry.id !== entryId).length
      },
      lastUpdated: new Date().toISOString().split('T')[0]
    }));
  };
  ...
  const handleConditionChange = (entryId, condition, value) => {
    setDataCollectionData(prev => ({
      ...
      dataEntries: prev.dataEntries.map(entry =>
        entry.id === entryId ? {
          ...entry,
          conditions: { ...entry.conditions, [condition]: value }
        } : entry
      ),
      lastUpdated: new Date().toISOString().split('T')[0]
    }));
  ...
  const validateData = () => {
    ...
    if (validValues.length > 0) {
      ...
      const stdDev = Math.sqrt(validValues.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / (validValues.length - 1));
  ...
  const addValidationRule = () => {
    const newRule = {
      id: Date.now(),
      type: 'range', // 'range', 'format', 'logic', 'custom'
      description: '',
      condition: '',
      action: 'flag', // 'flag', 'reject', 'warn'
      enabled: true
    };
  ...
  const addQualityIssue = () => {
    const newIssue = {
      id: Date.now(),
      timestamp: new Date().toISOString(),
      type: 'accuracy', // 'accuracy', 'completeness', 'consistency', 'timeliness', 'validity'
      description: '',
  ...
  return (
    <div className={styles.rcaContainer}>
      ...
      <div className={styles.mainContent}>
        {/* Top Section: Collection Information + AI Helper - Exact match to other tools */}
            ...
            <div className={styles.chatCard}>
              ...
              <div className={styles.chatInput}>
                <input
                  ...
                  placeholder="Ask me about data collection..."
              ...
              <div className={styles.quickActions}>
                ...
                <div className={styles.actionButtons}>
                  <button 
                    ...
                    onClick={() => handleQuickAction('What is data quality?')}
                  >
                    Data Quality
                  </button>
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction('How do I validate data?')}
                  >
                    Data Validation
                  </button>
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction('What about outliers?')}
                  >
                    Outlier Detection
                  </button>
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction('How much data do I need?')}
                  >
                    Sample Size
                  </button>
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction('What about missing data?')}
                  >
                    Missing Data
                  </button>
                </div>
            ...
            <div className={styles.fieldRow}>
              ...
              <div className={styles.fieldGroup}>
                ...
                <input
                  ...
                  onChange={(e) => handleSetupChange('targetSampleSize', parseInt(e.target.value) || 30)}
                  min="1"
                />
              </div>
            </div>
            ...
            <div className={styles.dimensionsGrid}>
              <div className={styles.dimensionCard}>
                ...
                <div className={styles.dimensionBar}>
                  <div 
                    ...
                    style={{ width: `${dataCollectionData.dataQuality.completeness}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className={styles.dimensionCard}>
                ...
                <div className={styles.dimensionBar}>
                  <div 
                    className={styles.dimensionFill} 
                    style={{ width: `${dataCollectionData.dataQuality.accuracy}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className={styles.dimensionCard}>
                ...
                <div className={styles.dimensionBar}>
                  <div 
                    className={styles.dimensionFill} 
                    style={{ width: `${dataCollectionData.dataQuality.consistency}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className={styles.dimensionCard}>
                ...
                <div className={styles.dimensionBar}>
                  <div 
                    className={styles.dimensionFill} 
                    style={{ width: `${dataCollectionData.dataQuality.timeliness}%` }}
                  ></div>
                </div>
              </div>
              ...
              <div className={styles.dimensionCard}>
                ...
                <div className={styles.dimensionBar}>
                  <div 
                    className={styles.dimensionFill} 
                    style={{ width: `${dataCollectionData.dataQuality.validity}%` }}
                  ></div>
                </div>
              </div>
            </div>
                ...
                {dataCollectionData.dataQuality.issues.map((issue) => (
                  <tr key={issue.id}>
                    ...
                    <td>
                      <button
                        className={styles.removeBtn}
                        onClick={() => removeQualityIssue(issue.id)}
                      >
                        <i className="fas fa-times"></i>
                      </button>
                    </td>
              ...
              <div className={styles.progressVisualization}>
                <div className={styles.progressBarLarge}>
                  <div 
                    ...
                    style={{ 
                      width: `${Math.min((dataCollectionData.collectionSetup.currentSampleSize / dataCollectionData.collectionSetup.targetSampleSize) * 100, 100)}%` 
                    }}
                  ></div>
                </div>
                <div className={styles.progressLabels}>
                  ...
                  <span>
                    {Math.round((dataCollectionData.collectionSetup.currentSampleSize / dataCollectionData.collectionSetup.targetSampleSize) * 100)}%
                  </span>
                </div>
              </div>
            </div>
