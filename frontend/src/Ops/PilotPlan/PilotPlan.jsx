...
const PilotPlan = () => {
  ...
  const [pilotData, setPilotData] = useState({
      ...
      phases: [
        {
          ...
          deliverables: '',
          responsible: '',
          status: 'not-started' // 'not-started', 'in-progress', 'completed', 'delayed'
        },
        {
          id: 2,
          name: 'Implementation',
      ...
      stakeholders: [
        {
          ...
          communicationMethod: 'email', // 'email', 'meeting', 'report', 'dashboard'
    ...
    results: {
      pilotStatus: 'planning', // 'planning', 'in-progress', 'completed', 'cancelled'
      actualStartDate: '',
      actualEndDate: '',
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      id: 1,
      type: 'ai',
      content: "Welcome to Pilot Plan! I'll help you design and plan a pilot test for your improvement solution. A well-designed pilot reduces implementation risk, validates your solution, and builds confidence for full-scale rollout. Start by defining your pilot objectives and success criteria. What solution are you planning to pilot?",
      timestamp: new Date()
    }
  ]);
  ...
  const removeMetric = (metricId) => {
    if (pilotData.successMetrics.length > 1) {
      setPilotData(prev => ({
        ...prev,
        successMetrics: prev.successMetrics.filter(metric => metric.id !== metricId),
        lastUpdated: new Date().toISOString().split('T')[0]
      }));
    }
  };
  ...
  const removeRisk = (riskId) => {
    if (pilotData.riskManagement.riskAssessment.length > 1) {
      setPilotData(prev => ({
        ...
        riskManagement: {
          ...prev.riskManagement,
          riskAssessment: prev.riskManagement.riskAssessment.filter(risk => risk.id !== riskId)
        },
        lastUpdated: new Date().toISOString().split('T')[0]
      }));
    }
  ...
  const removeStakeholder = (stakeholderId) => {
    if (pilotData.communicationPlan.stakeholders.length > 1) {
      setPilotData(prev => ({
        ...
        communicationPlan: {
          ...prev.communicationPlan,
          stakeholders: prev.communicationPlan.stakeholders.filter(stakeholder => stakeholder.id !== stakeholderId)
        },
        lastUpdated: new Date().toISOString().split('T')[0]
      }));
    }
  ...
  const generateSampleData = () => {
    const sampleData = {
      ...
      solutionDetails: {
        ...
        description: 'Implementation of visual work instructions, standard operating procedures, and quality checkpoints',
        category: 'process',
      ...
      pilotDesign: {
        ...
        randomization: false,
        blindingType: 'single',
        dataCollection: 'Daily defect counts, cycle time measurements, weekly satisfaction surveys'
      },
      timeline: {
  ...
  const generateAIResponse = (userMessage) => {
    const responses = {
      'pilot': "A good pilot tests your solution on a small scale before full implementation. Key elements: Clear objectives, success criteria, appropriate sample size, control group (if possible), data collection plan, timeline, and rollback plan. Start with low-risk, high-impact areas.",
      'design': "Pilot design depends on your solution type. Parallel pilots run alongside current process, sequential pilots replace current process temporarily, before-after compares results over time. Consider sample size, randomization, control groups, and blinding to reduce bias.",
      'metrics': "Choose metrics that directly relate to your problem statement and solution hypothesis. Include leading indicators (process metrics) and lagging indicators (outcome metrics). Set realistic targets based on baseline data and improvement potential.",
      'timeline': "Typical pilot phases: Preparation (planning, training, setup), Implementation (solution deployment), Monitoring (data collection, adjustments), Evaluation (analysis, decision). Allow enough time to see meaningful results but not so long that you lose momentum.",
      'risks': "Common pilot risks: Insufficient sample size, Hawthorne effect, resistance to change, data collection issues, external factors. Develop mitigation strategies and contingency plans. Have clear go/no-go criteria and rollback procedures.",
      'communication': "Keep stakeholders informed throughout the pilot. Regular updates build support and address concerns early. Tailor communication to audience - executives want results, operators want practical details, customers want minimal disruption.",
      'success': "Define success criteria upfront - quantitative targets and qualitative indicators. Consider statistical significance, practical significance, and sustainability. Success isn't just meeting targets - it's proving the solution works and can be scaled.",
      'scale': "If pilot succeeds, plan for scale-up: Resource requirements, training needs, change management, timeline, risk mitigation. If pilot fails, analyze why - was it the solution, implementation, or external factors? Use learnings for next iteration.",
      'default': "I can help with any aspect of pilot planning. Ask about pilot design, success metrics, timeline planning, risk management, or scaling strategies. What would you like to know?"
    };
  ...
  return (
    <div className={styles.pilotContainer}>
      ...
      <div className={styles.mainContent}>
        {/* Top Section - Pilot Info + AI Chat side by side */}
        <div className={styles.topSection}>
          {/* Pilot Information Card (Left Column) */}
            ...
            <div className={styles.chatCard}>
              ...
              <div className={styles.chatInput}>
                <input
                  ...
                  placeholder="Ask about pilot planning..."
                />
                <button 
              ...
              <div className={styles.quickActions}>
                ...
                <div className={styles.actionButtons}>
                  <button 
                    ...
                    onClick={() => handleQuickAction("How do I design an effective pilot?")}
                  >
                    Pilot Design
                  </button>
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction("What success metrics should I track?")}
                  >
                    Success Metrics
                  </button>
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction("How long should my pilot run?")}
                  >
                    Timeline Planning
                  </button>
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction("What risks should I consider?")}
                  >
                    Risk Management
                  </button>
                </div>
        ...
        <div className={styles.metricsCard}>
          ...
          <div className={styles.metricsGrid}>
            {pilotData.successMetrics.map((metric) => (
              <div key={metric.id} className={styles.metricCard}>
                <div className={styles.metricHeader}>
                  <h3>Metric {pilotData.successMetrics.indexOf(metric) + 1}</h3>
        ...
        <div className={styles.riskCard}>
          ...
          <div className={styles.riskGrid}>
            {pilotData.riskManagement.riskAssessment.map((risk) => (
              <div key={risk.id} className={styles.riskItemCard}>
                <div className={styles.riskHeader}>
                  <h3>Risk {pilotData.riskManagement.riskAssessment.indexOf(risk) + 1}</h3>
              ...
              <div className={styles.fieldGroup}>
                ...
                <textarea
                  ...
                  onChange={(e) => setPilotData(prev => ({
                    ...
                    riskManagement: {
                      ...prev.riskManagement,
                      escalationProcedure: e.target.value
                    }
                  }))}
        ...
        <div className={styles.communicationCard}>
          ...
          <div className={styles.stakeholdersGrid}>
            {pilotData.communicationPlan.stakeholders.map((stakeholder) => (
              <div key={stakeholder.id} className={styles.stakeholderCard}>
                <div className={styles.stakeholderHeader}>
                  <h3>Stakeholder {pilotData.communicationPlan.stakeholders.indexOf(stakeholder) + 1}</h3>
              ...
              <div className={styles.fieldGroup}>
                ...
                <textarea
                  ...
                  onChange={(e) => setPilotData(prev => ({
                    ...
                    communicationPlan: {
                      ...prev.communicationPlan,
                      reportingSchedule: e.target.value
                    }
                  }))}
