...
const RunChart = () => {
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      id: 1,
      type: 'ai',
      content: "Welcome to Run Chart Analysis! I'll help you create and analyze run charts to identify trends, patterns, and process behavior over time. Run charts are excellent for detecting non-random patterns and understanding process stability. What process or variable would you like to analyze?",
      timestamp: new Date()
    }
  ]);
  ...
  const generateAIResponse = (userMessage) => {
    const responses = {
      'trend': "Trends in run charts indicate systematic changes over time. Look for 6+ consecutive points consistently increasing or decreasing. Trends suggest process drift, tool wear, operator learning, or environmental changes.",
      'pattern': "Common run chart patterns include: trends (6+ points up/down), shifts (8+ points above/below median), runs (8+ consecutive points on same side), cycles (repeated patterns), and clustering (points grouped together).",
      'shift': "Shifts are 8 or more consecutive points on the same side of the median line. They indicate a change in process level, often due to process adjustments, material changes, or operational modifications.",
      'run': "Runs are sequences of consecutive points on the same side of the median. 8+ consecutive points suggest non-random behavior. Shorter runs are normal variation.",
      'cycle': "Cycles are repeated patterns that occur at regular intervals. They may indicate seasonal effects, shift changes, batch variations, or equipment maintenance cycles.",
      'outlier': "Outliers are points that appear unusually high or low compared to other data. Investigate special causes like equipment malfunctions, measurement errors, or unusual conditions.",
      'median': "The median line represents the middle value when data points are arranged in order. It's less sensitive to outliers than the mean and helps identify shifts and runs.",
      'stability': "Process stability is indicated by random variation around the median with no trends, shifts, runs, or cycles. Stable processes are predictable and in statistical control.",
      'interpretation': "Interpret run charts by: 1) Calculating median, 2) Identifying patterns (trends, shifts, runs, cycles), 3) Investigating special causes, 4) Determining process stability, 5) Planning improvements.",
      'default': "I can help with trend analysis, pattern identification, shift detection, run analysis, cycle recognition, outlier investigation, median calculation, stability assessment, or chart interpretation. What aspect needs guidance?"
    };
  ...
  useEffect(() => {
    if (chartData.dataPoints.length > 0) {
      const values = chartData.dataPoints.map(point => parseFloat(point.value)).filter(val => !isNaN(val));
  ...
  const removeDataPoint = (pointId) => {
    setChartData(prev => ({
      ...prev,
      dataPoints: prev.dataPoints.filter(point => point.id !== pointId).map((point, index) => ({
        ...point,
        sequence: index + 1
      })),
      lastUpdated: new Date().toISOString().split('T')[0]
            ...
            <div className={styles.chatCard}>
              ...
              <div className={styles.chatInput}>
                <input
                  ...
                  placeholder="Ask me about run chart analysis..."
              ...
              <div className={styles.quickActions}>
                ...
                <div className={styles.actionButtons}>
                  <button 
                    ...
                    onClick={() => handleQuickAction('How do I identify trends in run charts?')}
                  >
                    Trend Analysis
                  </button>
                  <button 
                  ...
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction('How do I detect shifts?')}
                  >
                    Shift Detection
                  </button>
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction('What are runs in run charts?')}
                  >
                    Run Analysis
                  </button>
                  <button 
                    className={styles.quickBtn}
                    onClick={() => handleQuickAction('How do I interpret my chart?')}
                  >
                    Interpretation
                  </button>
                </div>
